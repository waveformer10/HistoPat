  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        let topicosCount = 0;
        const slidesByTopicPromises: Promise<number>[] = [];
        for (const modulo of modulos) {
          const topicos = await findTopicsByModuleId(modulo.id);
          topicosCount += topicos.length;

          for (const topico of topicos) {
            slidesByTopicPromises.push(
              findSlidesBySubTopicId(topico.id).then((slides: any) => slides.length)
            );
          }
        }
        setTotalTopicos(topicosCount);

        const slidesCounts = await Promise.all(slidesByTopicPromises);
        const totalSlidesCount = slidesCounts.reduce((a, b) => a + b, 0);
